...
Daily DEX spot volume for Ethereum, Solana, Robinhood Chain, Base, BNB Chain, Arbitrum and HyperEVM, with an estimated market-cap split and the 30-day change. Automatic daily scheduling is paused during the paid-API end-to-end test; publication depends on successful fetching and validation.
...
- `.github/workflows/refresh.yml` — the refresh workflow, currently manual with an optional delayed start.
...
2. Add `COINGECKO_PRO_API_KEY` under repository Settings → Secrets and variables → Actions, then dispatch `refresh-volume-data` on `main`. The optional `not_before_utc` input sets an earliest refresh time in ISO UTC, at most 20 minutes ahead; a late runner starts immediately. The workflow requires the paid secret and fails before fetching if it is missing. It pins one UTC date window, restores compatible raw responses, fetches and validates all chains, then publishes only generated data. Daily scheduling remains paused until the test is confirmed; the intended daily time is 14:00 UTC.
...
A CoinGecko key configured only in Vercel does not reach GitHub Actions. Vercel hosts the static dashboard; the collector runs on the GitHub runner. The workflow does not repeat after a manual test until a schedule is explicitly restored.
